Keyword driven framework

test cases will be read from csv rather excel 
web automation using selenium


FEATURE:
1. to run single case														- add unique suite name and run as per suite
2. to run a particular page cases 											- done
3. to run particular suites													- done
4. to run combination of classes - home, search
5. to run combination of suite - sanity, regression
6. to run group of functionality, no need to write test case from scratch
   e.g go to search page
   apply price filter 100 200
7. reports  - update excel with result in each step
8. run from mvn commands
9. should handle large test cases - no out of memory  						-done
10. create property file to get basic things - driver, base url				- suite to run etc
   
   
   
   
Note:
currently we are writing low level test cases in testcases sheet like enter text,click etc   
along with these steps we should also have high level steps like login into site, create account with details etc which are high level modular methods


